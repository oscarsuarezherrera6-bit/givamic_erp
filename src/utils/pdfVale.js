...
// Colores del formato SIG-FO-103
...
export function generarPDFVale(vale, sedes, logo) {
  // A4 Portrait: 210 x 297 mm
  const doc = new jsPDF({ orientation: 'portrait', unit: 'mm', format: 'a4' })
  const W = 210
  const MG = 10
  const CW = W - 2 * MG  // 190mm
  let y = MG
  ...
  const LOGO_W = 38
  const CODIGO_W = 42
  const TITULO_W = CW - LOGO_W - CODIGO_W
  ...
  doc.text('SISTEMA INTEGRADO DE GESTIÓN', TX + TITULO_W / 2, y + 9, { align: 'center' })
  doc.setFont('helvetica', 'normal'); doc.setFontSize(8)
  doc.text('FORMATO DE SALIDA Y ENTREGA DE REQUERIMIENTOS', TX + TITULO_W / 2, y + 16, { align: 'center' })

  // Código/Versión/Fecha (columna derecha con 3 sub-filas)
  ...
  const vals = ['SIG-FO-103', '001', fmtDatePDF(vale.fecha)]
  lbls.forEach((lbl, i) => {
  ...
  const sedeNombre = sedes.find(s => s.id === vale.sedeDestinoId)?.nombre || ''
  const DG_H = 7
  const half = CW / 2

  const dgRows = [
    { l: 'AREA SOLICITANTE:', lv: vale.areaSolicitante, r: 'AREA QUE ENTREGA:', rv: 'Almacén Central' },
    { l: 'RESP. SOLICITANTE:', lv: vale.responsable, r: 'SEDE/AREÁ DE DESTINO:', rv: sedeNombre },
    { l: 'N° DE SALIDA:', lv: vale.numeroVale, r: null }
  ]
  ...
  // ── TABLA DE ÍTEMS ────────────────────────────────────────────────
  // Columnas: N° | DESCRIPCIÓN DEL BIEN | CODIGO | UM | CANT. REQ. | OBSERVACIONES
  const rawCols = [
    { h: 'N°',                            w: 8,  align: 'center' },
    { h: 'DESCRIPCIÓN DEL BIEN',           w: 64, align: 'left'   },
    { h: 'CODIGO',                         w: 22, align: 'center' },
    { h: 'UM',                             w: 13, align: 'center' },
    { h: 'CANT.\nREQ.',                    w: 17, align: 'center' },
    { h: 'OBSERVACIONES Y/O COMENTARIOS',  w: 66, align: 'left'   },
  ]
  // Escalar para que sumen exactamente CW
  ...
  for (let r = 0; r < MAX_ROWS; r++) {
    const it = vale.items[r] || null
    const ry = y + r * ROW_H

    if (r % 2 === 1) {
      doc.setFillColor(...GRIS_FILA)
      doc.rect(MG, ry, CW, ROW_H, 'F')
    }
    ...
    let lx = MG
    cols.forEach((col, ci) => {
      if (ci < cols.length - 1) doc.line(lx + col.w, ry, lx + col.w, ry + ROW_H)
      lx += col.w
    })
  ...
  doc.save(`Vale_Salida_${vale.numeroVale}_${sedeSlug}_${vale.fecha}.pdf`)
}
